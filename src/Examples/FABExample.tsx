...
import { FAB, Portal, useTheme, Text, Provider } from 'react-native-paper';
...
const FABExample = (): JSX.Element => {
  const [ visible, setVisible ] = React.useState<boolean>(true);
  const [ open, setOpen ] = React.useState<boolean>(false);
  
  const theme = useTheme();

  const variants: string[] = [ 'primary', 'secondary', 'tertiary', 'surface' ];
  const sizes: string[] = [ 'small', 'medium', 'large' ];
  const modes: string[] = [ 'flat', 'elevated' ];
